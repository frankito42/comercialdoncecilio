...
async function interesTarjeta(titulo = "Interes tarjeta",porcentajeInteres=5){
    ...
    input2.value=separator(numeroTotalMasInteres.toFixed(0))
    input2.type="number"
    input2.style.width="87px"
    input2.addEventListener("change",async ()=>{
        this.value=separator(numeroTotalMasInteres.toFixed(0))
       await sumarTodo()
    })
    input2.addEventListener("keyup",async ()=>{
        this.value=separator(numeroTotalMasInteres.toFixed(0))
    ...
    input3.value=0
    input3.style.display="none"
    textoCelda1 = document.createTextNode(titulo);
    /* console.log(mayoriOminori) */
